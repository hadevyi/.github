...
기존 각 저장소의 매월 13일 오전 9시 8분 실행은 미리보기만 수행합니다. `mode`를 생략한 기존 호출도 원격을 변경하지 않습니다.
...
사용 가능한 프로필과 기존 별칭은 [라벨 체계](labels.md)에서 관리합니다. 예시의 `@main`은 기존 호출 호환용이며, 이후 개별 호출 갱신 시 검증한 전체 Commit SHA로 고정합니다.

적용하려면 수동 호출에 `mode: apply`와 같은 저장소의 관련 열린 `issue_number`를 함께 전달합니다. 예약·PR 실행에서는 적용을 거부합니다. 생성·색상·설명 갱신만 허용하며 기존 라벨 삭제·이름 이전은 수행하지 않습니다.
...
3. 미리보기의 생성·갱신·보존 목록을 확인합니다.
4. 기존 Issue·PR 연결의 이전 계획을 별도로 검증한 뒤 필요한 생성·갱신을 명시적으로 실행합니다.

동기화에 라벨 삭제나 이름 변경 기능은 없습니다. 기존 기록 이전과 제거는 별도 작업으로 추적합니다.
