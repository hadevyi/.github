...
| 공통 라벨 동기화 | `reusable-sync-labels.yml` | 공통·프로필별 라벨 생성 및 갱신 |
...
3. 라벨 동기화를 수동 실행해 기존 라벨과 결과를 확인합니다.
4. 기존 이슈의 라벨을 새 체계로 전환합니다.
5. 사용하지 않는 기존 라벨을 확인 후 삭제합니다.

중앙 라벨 동기화는 저장소 전용 라벨을 자동 삭제하지 않습니다.
